import { HStack, IconButton, Tag, TagCloseButton, TagLabel, Text, Tooltip, Wrap, Button } from "@chakra-ui/react";
import { LuCrosshair, LuSquare, LuHexagon } from "react-icons/lu";
...
/**
 * The marker toolbar above the Chamber Camera video: shape tools, the list of
 * markers already drawn (click to select, x to remove), and the jump to their
 * calibration trace.
 *
...
const FiducialToolbar = ({ markers }: Props) => {
  ...
  const focusPanel = useFocusPanel();
  ...
  return (
    <HStack justify="space-between" flexWrap="wrap" rowGap={1} className="no-drag">
      ...
      <Wrap spacing={1} flex="1" minW="120px">
        {markers.map((marker) => (
          <Tag
            ...
            colorScheme={marker.marker_id === selectedMarkerId ? "orange" : "green"}
            cursor="pointer"
            onClick={() =>
      ...
      <Button
        size="xs"
        variant="panelGhost"
        isDisabled={!selectedMarkerId}
        onClick={() => focusPanel("fiducialTrace")}
      >
        View calibration trace →
      </Button>
    </HStack>
  );
};
